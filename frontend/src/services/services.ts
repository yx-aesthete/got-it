...
export const voteOnQuestionSocket = (className: any, topicName: any, questionId: any, answerIndex: any) => {
...
export const incrementCurrentTopic = async (classId: any) => {
  try {
    const response = await axios.post(`${API_BASE_URL}/incrementCurrentTopic`, { classId });
    return response.data;
  } catch (error) {
...
export const addQuestionToTopic = async (className: any, topicName: any, questionId: any) => {
  try {
    const response = await axios.post(`${API_BASE_URL}/addQuestionByIdToTopic`, {
      className,
      topicName,
      questionId,
    });
